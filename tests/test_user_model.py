...
class UserModelTestCase(unittest.TestCase):
    ...
    def test_invalid_reset_token(self):
        ...
        db.session.commit()
        token = u.generate_reset_token()
        self.assertFalse(User.reset_password(token + 'a', 'hose'))
        self.assertTrue(u.verify_password('cat'))

    def test_valid_email_change_token(self):
        u = User(email='Alex@example.com', password='cat')
        db.session.add(u)
        db.session.commit()
    ...
    def test_invalid_email_change_token(self):
        u1 = User(email='Alex@example.com', password='cat')
        u2 = User(email='susan@example.com', password='dog')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
        token = u2.generate_email_change_token('Alex@example.com')
